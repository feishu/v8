...
class V8_EXPORT String : public Name {
 public:
  ...
  uint32_t Utf8LengthV2(Isolate* isolate) const;
   ...
   */
  void WriteV2(Isolate* isolate, uint16_t* buffer, uint32_t length,
               uint32_t offset = 0, int flags = WriteFlags::kNone) const;
  void WriteOneByteV2(Isolate* isolate, uint8_t* buffer, uint32_t length,
                      uint32_t offset = 0, int flags = WriteFlags::kNone) const;
   ...
   * output buffer unless either all characters were encoded or the buffer is
   * full. Will not write partial UTF-8 sequences, preferring to stop before
   * the end of the buffer. The required size of the output buffer can be
   * determined using Utf8Length().
   *
   * \param buffer The buffer into which the string will be copied.
   * \param capacity The number of bytes in the output buffer.
   * \param flags Various flags that influence the behavior of this operation.
   * \return The number of bytes copied to the buffer including the null
   * terminator (if written).
   */
  uint32_t WriteUtf8V2(Isolate* isolate, char* buffer, uint32_t capacity,
