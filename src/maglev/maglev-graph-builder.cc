...
bool MaglevGraphBuilder::CheckContextExtensions(size_t depth) {
  ...
  // PrintF("=== Maglev: generating context extension checks\n");
  for (uint32_t d = 0; d < depth; d++) {
    ...
    if (scope_info.HasContextExtensionSlot()) {
      if (broker()->dependencies()->DependOnEmptyContextExtension(scope_info)) {
        // PrintF("=== Maglev: depending on EmptyContextExtension\n");
      } else {
        ...
        // PrintF("=== Maglev: generate dynamic EmptyContextExtension check\n");
      }
    }
