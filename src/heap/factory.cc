...
Handle<JSFunction> Factory::NewDecoratorAddInitializerFunction(
  ...
  Handle<Context> add_initializer_context = factory->NewBuiltinContext(
      isolate->native_context(),
      DecoratorsBuiltins::kExtraInitializersContextLength);
  add_initializer_context->set(DecoratorsBuiltins::kInitializersListContextSlot,
...
Factory::NewJSClassElementDecoratorContextObject(
    ...
    Handle<JSDecoratorAccessObject> decorator_access_object,
    DirectHandle<Boolean> is_static, DirectHandle<Boolean> is_private,
    DirectHandle<String> name, Handle<JSFunction> add_initializer_function) {
