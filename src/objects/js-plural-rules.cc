...
Handle<JSObject> JSPluralRules::ResolvedOptions(
  ...
  // 6. Let pluralCategories be a List of Strings containing all possible results
  // of PluralRuleSelect for the selected locale pr.[[Locale]], sorted according
  // to the following order: "zero", "one", "two", "few", "many", "other".
  icu::PluralRules* icu_plural_rules = plural_rules->icu_plural_rules()->raw();
  DCHECK_NOT_NULL(icu_plural_rules);
