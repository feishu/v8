...
size_t String::EncodeUtf8(AllowNullsFlag allow_nulls, char* buffer,
                          size_t capacity, uint32_t offset, uint32_t length,
                          bool null_terminate, bool ensure_valid) {
  DCHECK_LE(length, this->length());
  DCHECK_LE(offset + length, this->length());
...
std::unique_ptr<char[]> String::ToCString(AllowNullsFlag allow_nulls,
  ...
  size_t written = EncodeUtf8(allow_nulls, result, capacity, offset, length);
