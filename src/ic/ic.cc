...
bool KeyedStoreIC::CanTransitionAllocationSiteToStayMonomorphic(
    ...
    Handle<Map> new_receiver_map) {
  if (receiver_map.is_null()) return false;      // FIXME: can this happen?
  if (new_receiver_map.is_null()) return false;  // FIXME: can this happen?
  if (new_receiver_map->is_abandoned_prototype_map())
    return false;  // FIXME: can this happen?
