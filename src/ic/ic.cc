...
// updated to `ic_map`, we can keep the IC monomorphic.
bool KeyedStoreIC::CanUpdateAllocationSiteToStayMonomorphic(
  ...
  // *receiver_map == *ic_map happen if the store already transitioned the
  // receiver. If there's an AllocationSite, it has been updated already.
  ...
  DirectHandle<Map> new_receiver_map =
      JSObject::GetElementsTransitionMap(receiver_jsobject, ic_elements_kind);

  if (*new_receiver_map != *ic_map) return false;

  // If the store already transitioned the receiver, we don't need any updates.

  constexpr bool kReturnTrueIfAlreadyUpdated = true;
  if (!JSObject::UpdateAllocationSite(receiver_jsobject, ic_elements_kind,
                                      kReturnTrueIfAlreadyUpdated))
    return false;
...
void KeyedStoreIC::UpdateStoreElement(Handle<Map> old_receiver_map,
  ...
  if (state() == MONOMORPHIC) {
    ...
    if (IsTransitionOfMonomorphicTarget(*previous_receiver_map,
      ...
      return;
    } else if (CanUpdateAllocationSiteToStayMonomorphic(previous_receiver_map,
                                                        receiver)) {
