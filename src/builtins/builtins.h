...
// for it. For example ISX_Builtin::kBuiltin_A equals to Builtin::kBuiltin_A.
enum class ISX_Builtin : int32_t {
#define DEF_ENUM(Name, ...) k##Name = static_cast<int32_t>(Builtin::k##Name),
  ISX_BUILTIN_LIST(DEF_ENUM)
#undef DEF_ENUM
};

// We build a separate list for the Builtins that need ISX. These Builtins are
// ordered by their original Builtin id, and the enum class
// represent their order in the list. For example, if we take
// Builtin::kBuiltin_A as ISX Builtin with smallest Builtin id,
// ISX_Builtin_order::kBuiltin_A is set to 0.
enum class ISX_Builtin_order : int32_t {
#define DEF_ENUM(Name, ...) k##Name,
  ISX_BUILTIN_LIST(DEF_ENUM)
#undef DEF_ENUM
};
...
#define ADD_ONE(Name, ...) +1
  ...
  static constexpr int kISXBuiltinCount = 0 ISX_BUILTIN_LIST(ADD_ONE);
#else
  static constexpr int kISXBuiltinCount = 0;
...
#if V8_ENABLE_ISX_BUILTIN
  static constexpr int32_t GetISXBuiltinOrder(Builtin builtin) {
    switch (static_cast<int32_t>(builtin)) {
#define DEF_ENUM(Name, ...)                        \
  case static_cast<int32_t>(ISX_Builtin::k##Name): \
    return static_cast<int32_t>(ISX_Builtin_order::k##Name);
      ISX_BUILTIN_LIST(DEF_ENUM)
      default:
        return -1;
    }
#undef DEF_ENUM
  ...
  static constexpr bool IsBuiltinNeedsISX(Builtin builtin) {
    return GetISXBuiltinOrder(builtin) != -1;
  }
#endif  // V8_ENABLE_ISX_BUILTIN
