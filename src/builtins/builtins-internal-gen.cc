...
class WriteBarrierCodeStubAssembler : public CodeStubAssembler {
 public:
  ...
  void InsertIntoRememberedSet(TNode<IntPtrT> object, TNode<IntPtrT> slot,
    ...
    TNode<IntPtrT> max_buckets_address =
        IntPtrSub(slot_set, IntPtrConstant(sizeof(size_t)));
    TNode<IntPtrT> max_buckets = UncheckedCast<IntPtrT>(
        Load(MachineType::Pointer(), max_buckets_address, IntPtrConstant(0)));

    // Load bucket
    TNode<IntPtrT> bucket =
        LoadBucket(slot_set, slot_offset, max_buckets, &slow_path);
  ...
  TNode<IntPtrT> LoadBucket(TNode<IntPtrT> slot_set, TNode<WordT> slot_offset,
                            TNode<IntPtrT> max_buckets, Label* slow_path) {
    TNode<WordT> bucket_index =
        WordShr(slot_offset, SlotSet::kBitsPerBucketLog2 + kTaggedSizeLog2);
    CSA_CHECK(this, IntPtrLessThan(bucket_index, max_buckets));
