...
void JumpTableAssembler::EmitLazyCompileJumpSlot(uint32_t func_index,
  ...
  base::Memory<uint32_t>(pc_ + (0 * kInstrSize)) =
      inst[0] | Assembler::ImmMoveWide(func_index_low);
  base::Memory<uint32_t>(pc_ + (1 * kInstrSize)) =
      inst[1] | Assembler::ImmMoveWide(func_index_high);
  base::Memory<uint32_t>(pc_ + (2 * kInstrSize)) =
      inst[2] |
...
bool JumpTableAssembler::EmitJumpSlot(Address target) {
  ...
  reinterpret_cast<std::atomic<uint32_t>*>(pc_)->store(
      bti_inst, std::memory_order_relaxed);
  pc_ += kInstrSize;
#endif
  ...
  uint32_t branch_inst =
      ...
      Assembler::ImmUncondBranch(base::checked_cast<int32_t>(target_offset));
  reinterpret_cast<std::atomic<uint32_t>*>(pc_)->store(
      branch_inst, std::memory_order_relaxed);
  pc_ += kInstrSize;
...
void JumpTableAssembler::EmitFarJumpSlot(Address target) {
  ...
  base::Memory<uint32_t>(pc_ + (0 * kInstrSize)) = inst[0];
  base::Memory<uint32_t>(pc_ + (1 * kInstrSize)) = inst[1];
  base::Memory<Address>(pc_ + (2 * kInstrSize)) = target;
  pc_ += kFarJumpTableSlotSize;
