...
double pow(double x, double y) {
  // From https://tc39.es/ecma262/#sec-numeric-types-number-exponentiate:
  //
  // ===
  // Note:
  // The result of base ** exponent when base is 1𝔽 or -1𝔽 and exponent is +∞𝔽
  // or -∞𝔽, or when base is 1𝔽 and exponent is NaN, differs from IEEE 754-2019.
  // The first edition of ECMAScript specified a result of NaN for this
  // operation, whereas later revisions of IEEE 754 specified 1𝔽. The historical
  // ECMAScript behaviour is preserved for compatibility reasons.
  // ===
  //
  // We can special case these exceptions, and use std::pow (which implements
  // IEEE 754) otherwise.
  if (std::isnan(y)) {
    return std::numeric_limits<double>::quiet_NaN();
  }
  if (std::abs(x) == 1 && std::isinf(y)) {
    return std::numeric_limits<double>::quiet_NaN();
  }

#ifndef __STDC_IEC_559__
#error "The C library must support IEEE 754 (IEC 60559 / IEC 559:1989)"
#endif
  return std::pow(x, y);
}
